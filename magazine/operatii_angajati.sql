...
create or replace package operatii_angajati as
  function add_angajat(p_id_angajat in angajati.id%TYPE, p_nume_angajat angajati.nume%TYPE, p_prenume_angajat angajati.prenume%TYPE, p_functie angajati.functie%TYPE, p_adresa_magazin angajati.id_magazin%TYPE) return varchar2;
   function delete_angajat(p_id_angajat in angajati.id%TYPE) return varchar2;
   function update_angajat(p_id_angajat in angajati.id%TYPE, p_nume angajati.nume%TYPE, p_prenume angajati.prenume%TYPE) return varchar2;
...
create or replace package body operatii_angajati as
  ...
  function add_angajat(p_id_angajat in angajati.id%TYPE, p_nume_angajat angajati.nume%TYPE, p_prenume_angajat angajati.prenume%TYPE, p_functie angajati.functie%TYPE, p_adresa_magazin angajati.id_magazin%TYPE) return varchar2 as
    v_done_a number;
    v_done_p number;
    v_done varchar2(15);
  ...
  begin
    ...
    insert into angajati (id, id_magazin, nume, prenume, functie) values (p_id_angajat,p_adresa_magazin, p_nume_angajat, p_prenume_angajat, p_functie);
  ...
  function update_angajat(p_id_angajat in angajati.id%TYPE, p_nume angajati.nume%TYPE, p_prenume angajati.prenume%TYPE) return varchar2 as
    v_done_p number;
    v_done varchar2(15);
    v_exception_a number := 0;
  ...
  begin
    ...
    update angajati set nume = p_nume, prenume = p_prenume where id = p_id_angajat;
...
function interogare_angajat(p_id_angajat in angajati.id%TYPE)  return varchar2 as
  ...
  begin
    ...
    v_informatii := v_id_angajat || ' ' || v_nume_angajat || ' ' || v_prenume_angajat;
    return v_informatii;
    end if;
    exception
      when id_persoana_inexistent then
